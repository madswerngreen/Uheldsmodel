...
class Data:
    """
    Data handler for joint accident model.

    Responsible for:
        - Loading and preprocessing raw data
        - Constructing exposure variable
        - Feature engineering
        - Preparing design matrices (X, Z, W)

    Model variables:
        y   : total accidents (Personskade + Materielskade)
        y_p : injury accidents (Personskade)
        z   : severity counts [slight, serious, fatal]

    Design matrices:
        X : frequency model (NB)
        Z : injury probability model (Binomial)
        W : severity models (NB)

    Notes:
        - Exposure is measured in vehicle-kilometers
        - Only data from 2023 is used in default loading
        - Links with zero exposure are removed
    """
    ...
    def __init__(self, df=None, path="../Data/Model_Input/Uheld_LINKS.csv",
        ...
        if df is None:
            ...
            # Exposure (vehicle-km)
            # ----------------------------------------------------
            data['exposure_vehicle_km'] = (
                data['Traf_Car_Van'] + data['Traf_Truck']
            ) * data['Length']

            # Remove links with no traffic (no statistical information)
            data = data[data['exposure_vehicle_km'] > 0]

            # Restrict to 2023 (consistent with traffic input data)
            data = data[data['AAR'] == 2023]

            # ----------------------------------------------------
            # Speed handling
            # ----------------------------------------------------
            # If no vehicles observed, fallback to free-flow speed
            data.loc[data['Traf_Truck'] == 0,
                     'AvgSpeed_Truck'] = data['FreeSpeed']
            data.loc[data['Traf_Car_Van'] == 0,
                     'AvgSpeed_Car_Van'] = data['FreeSpeed']
            ...
            # Share of trucks (captures vehicle mix)
            data['Frac_Truck'] = data['Traf_Truck'] / total_traffic

            # Congestion proxies
            data['SpeedDiff_Car_Van'] = data['FreeSpeed'] - \
                data['AvgSpeed_Car_Van']
            data['SpeedDiff_Truck'] = np.minimum(
                data['FreeSpeed'], 80) - data['AvgSpeed_Truck']

            # ----------------------------------------------------
            # Accident rates (not used directly in likelihood)
            # ----------------------------------------------------
            data['Personskade_rate'] = data['Personskade'] / \
                data['exposure_vehicle_km']
            data['Materielskade_rate'] = data['Materielskade'] / \
                data['exposure_vehicle_km']
        ...
        if FEATURES_W is None:
            self.FEATURES_W = ['Link_group_0',
                               'Link_group_1', 'Link_group_2', 'Link_group_3']
        else:
            self.FEATURES_W = FEATURES_W

    def var_extract(self):
        """
        Extract model variables and design matrices.

        Returns:
            y         : total accidents
            y_p       : injury accidents
            z         : severity counts (n x 3)
            X, Z, W   : design matrices with intercept
            exposure  : vehicle-km exposure
        """
        ...
        y = (data['Personskade'] + data['Materielskade']).values
        ...
        X = build_matrix(data, self.FEATURES_X)
        Z = build_matrix(data, self.FEATURES_Z)
        W = build_matrix(data, self.FEATURES_W)

        return y, y_p, z, X, Z, W, exposure, LinkID
...
class Model:
    """
    Model class for hierarchical accident modeling.

    The model consists of three components:
    - Accident frequency modeled using a Negative Binomial (NB2) regression
    - Injury occurrence modeled using a Binomial model with logit link
    - Injury severity (slight, serious, fatal) modeled using separate
      Negative Binomial regressions

    The class provides:
    - Likelihood functions (NB, Binomial, ZINB) with gradients and Hessians
    - A joint posterior combining likelihoods and Gaussian priors
    - An optimization routine based on Empirical Bayes with Laplace approximation

    Designed for efficient estimation of hierarchical count models with
    overdispersion and exposure offsets.
    """
    ...
    def Binomial(gamma, y_total, y_person, Z, return_hessian=False):
        ...
        # X^T diag(w) X without forming diag
        hess = -(Z.T @ (Z * w[:, None]))
    ...
    """ NOT USED POSIBLE FUTURE EXTENSION
    @staticmethod
    ...
    # ----------------------------------------------------
    # 3. Posterior (sum of all log components)
    # ----------------------------------------------------
    def Posterior(self, params, sigmas, y, y_p, z, X, Z, W, exposure):
        ...
        ll_total = (
            ...
            + self.NB(delta_slight,  log_alpha_slight,
                      z[:, 0], W, offset=np.log(y_p + 1e-12))
            + self.NB(delta_serious, log_alpha_serious,
                      z[:, 1], W, offset=np.log(y_p + 1e-12))
            + self.NB(delta_fatal,   log_alpha_fatal,
                      z[:, 2], W, offset=np.log(y_p + 1e-12))
        )
    ...
    def optimize(self, y, y_p, z, X, Z, W, exposure, verbose=True, tol=1e-3, max_outer=50, damping=0.3):
        """
        MAP estimation with iterative hyperparameter tuning via moment matching.
        
        The optimization alternates between:
        1) MAP estimation of model parameters given current Gaussian priors
        2) Update of prior variances (sigmas) using moment matching based on
           a Laplace approximation of the posterior
        
        This corresponds to an Empirical Bayes (Type-II ML) procedure.
        """
        # param shapes
        k = X.shape[1]
        l = Z.shape[1]
        ...
        # --- Initialize parameters vector theta = [beta, log_alpha_y, gamma, delta_slight, log_alpha_slight, delta_serious, log_alpha_serious, delta_fatal, log_alpha_fatal]
        theta0 = np.zeros(n_params)
        theta0[:k] = 0.0
        theta0[0] = -5 # intercept (low baseline rate)
        ...
        for i in range(3):
            # each injury dispersion
            theta0[start + i*(m + 1) + m] = np.log(0.3)
        ...
        for outer in pbar:
            ...
            res = minimize(
                ...
                method="L-BFGS-B", # L-BFGS-B optimization algorithm
                tol=1e-8,
                options={'maxfun': 500000, 'maxiter': 500000}
            )
            theta_hat = res.x

            # Unpack MAP estimates for each block
            i = 0
            beta_hat = theta_hat[i:i+k]
            i += k
            loga_y = theta_hat[i]
            i += 1
            gamma_hat = theta_hat[i:i+l]
            i += l

            delta_s_hat = theta_hat[i:i+m]
            i += m
            la_s_hat = theta_hat[i]
            i += 1
            delta_se_hat = theta_hat[i:i+m]
            i += m
            la_se_hat = theta_hat[i]
            i += 1
            delta_f_hat = theta_hat[i:i+m]
            i += m
            la_f_hat = theta_hat[i]
            i += 1

            # ========== Log Likelihood Hessians per block at MAP ==========
            _, _, H_beta = self.NB(beta_hat,  loga_y,   y,      X, offset=np.log(
                exposure + 1e-12),  return_hessian=True)
            _, _, H_gamma = self.Binomial(
                gamma_hat,     y, y_p, Z,                                   return_hessian=True)
            _, _, H_s = self.NB(delta_s_hat,  la_s_hat,  z[:, 0], W, offset=np.log(
                y_p + 1e-12),   return_hessian=True)
            _, _, H_se = self.NB(delta_se_hat, la_se_hat, z[:, 1], W, offset=np.log(
                y_p + 1e-12),   return_hessian=True)
            _, _, H_f = self.NB(delta_f_hat,  la_f_hat,  z[:, 2], W, offset=np.log(
                y_p + 1e-12),   return_hessian=True)
            ...
            # update sigma^2 = E[theta^2] = Var(theta) + (E[theta])^2 for each block, excluding intercepts
            ...
            for kkey in sigmas:
                sigmas_new[kkey] = (1 - damping) * \
                    sigmas[kkey] + damping * sigmas_new[kkey]

            # Convergence check across all blocks
            diff = 0.0
            for kkey in sigmas:
                a = sigmas[kkey].ravel()
                b = sigmas_new[kkey].ravel()
                diff += np.sum((a - b)**2)

            if diff < tol:
                if verbose:
                    print(
                        f"EB converged in {outer+1} outer iters; ||Δσ||²={diff:.3e}")
                break

            # Prepare next EB iteration
            sigmas = sigmas_new
            theta0 = theta_hat.copy() # reuse theta_hat as theta_0 for next optimization iteration
        pbar.close()
        ...
        if verbose:
            ...
            print(
                f"Final objective (−log posterior): {res.fun:.3f}; inner iters: {res.nit}")
        # Also store final sigmas for inspection
        self.sigmas_ = sigmas
        self.theta_hat_ = theta_hat
        # --- Posterior standard deviations from last iteration ---
        self.sigma_post_ = {
            # include NaN for intercept
            "beta": np.concatenate([[np.nan], np.sqrt(var_beta)]),
    ...
    def predict_summary(self, y, y_p, z, X, Z, W, exposure, LinkID, feature_names_X, feature_names_Z, feature_names_W):
        """
        ...
        print("\n=== Prediction Summary ===")
        print(
            f"{'Component':<20}{'Observed':>15}{'Predicted':>15}{'Ratio (pred/obs)':>20}")
        print("-" * 70)
        print(
            f"{'Total accidents':<20}{obs_total:15.2f}{pred_total:15.2f}{pred_total/obs_total:20.3f}")
        ...
        print(
            f"{'Fatal injuries':<20}{obs_fatal:15.2f}{pred_fatal:15.2f}{pred_fatal/obs_fatal:20.3f}")
...
def cross_validate_joint(data_obj, K=5, max_iter=1000, out_dir="../Results/"):
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(X), 1):
        ...
        y_test, y_p_test, z_test, exp_test, LinkID_test = y[test_idx], y_p[
            test_idx], z[test_idx], exposure[test_idx], LinkID[test_idx]

        # --- Train model
        res = lik.optimize(y_train, y_p_train, z_train, X_train,
                           Z_train, W_train, exp_train, verbose=False)

        # --- Predict on test data
        pred_res = lik.predict_summary(y_test, y_p_test, z_test, X_test, Z_test, W_test, exp_test, LinkID_test,
        ...
        rmse_total = np.sqrt(
            np.mean((df_pred["y_obs"] - df_pred["y_pred"])**2))
        rmse_injury = np.sqrt(
            np.mean((df_pred["y_p_obs"] - df_pred["y_p_pred"])**2))
    ...
    df_all_pred.to_csv(os.path.join(
...
class RQR:
    """
    Computes randomized quantile residuals.

    Under correct model specification, RQRs should follow N(0,1),
    """

    @staticmethod
    def NB(y, mu, alpha, rng=None):
        """
        RQR for Negative Binomial distributed counts.

        Parameters
        ----------
        y : observed counts
        mu : predicted mean
        alpha : dispersion parameter (NB2)
        rng : random generator (for uniform draw)
        """
        if rng is None:
        ...
        # CDF bounds for discrete distribution
    ...
    def Binom(y_p, y_tot, p, rng=None):
        """
        RQR for Binomial distributed counts.

        Parameters
        ----------
        y_p : observed successes
        y_tot : number of trials
        p : predicted success probability
...
# ----------------------------------------
# Normality test class for RQR diagnostics
# ----------------------------------------
...
class tests:
    ...
    def t_test(rqr, only_p_val=False):
        """
        Test if mean of RQRs equals 0 (should hold under correct model).
        """
    ...
    def chi2_test(rqr, only_p_val=False):
        """
        Test if variance of RQRs equals 1 using chi-square distribution.
        """
        n = len(rqr)
        sample_var = np.var(rqr, ddof=1)
        stat = (n - 1) * sample_var  # scaled sample variance
        df = n - 1
...
def QQ_PLOT_ALL(params, pred,
                ...
                name='QQ-plot'):
    """
    Generate QQ-plot diagnostics based on Randomized Quantile Residuals (RQRs)
    for all model components across cross-validation folds.

    Parameters
    ----------
    params : pandas.DataFrame
        Output from `cross_validate_joint` containing parameter estimates.
        Must include columns:
            - 'fold'
            - 'alpha_y', 'alpha_slight', 'alpha_serious', 'alpha_fatal'

    pred : pandas.DataFrame
        Output from `cross_validate_joint` containing predictions.
        Must include columns:
            - 'fold'
            - observed values: 'y_obs', 'y_p_obs', 'z_slight_obs', ...
            - predicted values: 'y_pred', 'y_p_pred', 'z_slight_pred', ...

    path : str, optional
        Directory where the QQ-plot figure is saved.
        Default is "../Results/Figures".

    name : str, optional
        Filename of the saved plot ('.png' is added automatically if missing).
        Default is 'QQ-plot'.

    Returns
    -------
    None
        Saves a multi-panel QQ-plot figure showing residual diagnostics.

    Notes
    -----
    - RQRs are computed separately for each fold using fold-specific dispersion parameters.
    - Normality is assessed visually (QQ-plots) and via:
        * t-test (mean = 0)
        * chi-square test (variance = 1)
        * Kolmogorov-Smirnov test (distributional fit)
    - P-values below 0.05 are highlighted in red in the plots.
    """
    ...
    for f in pred["fold"].unique():
        ...
        subset["RQR_y_p"] = RQR.Binom(
            subset["y_p_obs"], subset["y_obs"], subset["y_p_pred"] / subset["y_pred"])
        subset["RQR_slight"] = RQR.NB(
            subset["z_slight_obs"], subset["z_slight_pred"], a_s)
        subset["RQR_serious"] = RQR.NB(
            subset["z_serious_obs"], subset["z_serious_pred"], a_se)
        subset["RQR_fatal"] = RQR.NB(
            subset["z_fatal_obs"], subset["z_fatal_pred"], a_f)
